...
def worker(variety):
    ...
    return False 拼接出错
    '''
    Merge.merge_zl_tick_data(database_dir, variety=variety, factor_name='')
    # Merge.append_zl_tick_data(database_dir, variety=variety, factor_name='')
    # Merge.append_zl_tick_data(database_dir, variety='AG', date_='20230323')
    '''
    如果是拼接因子数据，现版本需要手动输入因子名_窗口_作者，之后在上层叠加接口实现。
    如果factor_name为''则表示拼接行情数据；
    如果date_为空表示使用当天的数据，否则传入'YYYY-MM-DD'格式的日期。
    '''
    # Merge.append_zl_tick_data(database_dir, variety, date_='2023-03-11')
p_list = []
for variety in ['AG']:
